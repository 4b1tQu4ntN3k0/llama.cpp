...
llm_build_qwen3next_pipo::llm_build_qwen3next_pipo(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        wqkv               = get_offloaded(layer.wqkv);
        wqkv_gate          = get_offloaded(layer.wqkv_gate);
        ssm_in             = get_offloaded(layer.ssm_in);
        ssm_beta_alpha     = get_offloaded(layer.ssm_beta_alpha);
        ssm_dt             = get_offloaded(layer.ssm_dt);
        ssm_a              = get_offloaded(layer.ssm_a);
        ssm_conv1d         = get_offloaded(layer.ssm_conv1d);
        ssm_norm           = get_offloaded(layer.ssm_norm);
        ssm_out            = get_offloaded(layer.ssm_out);
        ...
        ffn_gate_up_exps   = get_offloaded(layer.ffn_gate_up_exps);
        ffn_up_shexp       = get_offloaded(layer.ffn_up_shexp);
        ffn_gate_shexp     = get_offloaded(layer.ffn_gate_shexp);
        ffn_down_shexp     = get_offloaded(layer.ffn_down_shexp);
        ffn_gate_inp_shexp = get_offloaded(layer.ffn_gate_inp_shexp);
        ...
        // Post-attention norm
        ggml_tensor * cur_post_norm = build_norm(cur, attn_post_norm, nullptr, LLM_NORM_RMS, il);
        cb(cur_post_norm, "attn_post_norm", il);

        // FFN layer (MoE or dense) - without residual connection
        cur = build_layer_ffn(cur_post_norm, il);
        cb(cur, "ffn_out", il);
...
ggml_tensor * llm_build_qwen3next_pipo::build_norm_gated(
        ggml_tensor * input,
        ggml_tensor * weights,
        ggml_tensor * gate,
        int           layer) {
...
ggml_tensor * llm_build_qwen3next_pipo::build_layer_attn(
    ...
    ggml_tensor * Qcur_full = build_lora_mm(wq, cur);
    ...
    Qcur = build_norm(Qcur, attn_q_norm, nullptr, LLM_NORM_RMS, il);
    cb(Qcur, "Qcur_normed", il);

    Kcur = build_norm(Kcur, attn_k_norm, nullptr, LLM_NORM_RMS, il);
...
std::pair<ggml_tensor *, ggml_tensor *> llm_build_qwen3next_pipo::build_qkvz(
                ggml_tensor * input,
                        int   il) {
    ...
    if (wqkv) {
        // optimized path
        ggml_tensor * qkv_mixed = build_lora_mm(wqkv, input);
        qkv_mixed = ggml_reshape_3d(ctx0, qkv_mixed, qkv_mixed->ne[0], n_seq_tokens, n_seqs);
        cb(qkv_mixed, "linear_attn_qkv_mixed", il);

        ggml_tensor * z = build_lora_mm(wqkv_gate, input);
        cb(z, "z", il);

        return { qkv_mixed, z };
    } else {
        // legacy (slower) path
        ggml_tensor * mixed_qkvz = build_lora_mm(ssm_in, input);
...
ggml_tensor * llm_build_qwen3next_pipo::build_layer_attn_linear(
    ...
    ggml_tensor * mixed_ba = build_lora_mm(ssm_beta_alpha, cur);
    ...
    ggml_tensor * conv_kernel      = ssm_conv1d;
    ...
    ggml_tensor * attn_out_norm = build_norm_gated(output, ssm_norm, z_2d, il);
    ...
    // Output projection
    cur = build_lora_mm(ssm_out, final_output);
...
ggml_tensor * llm_build_qwen3next_pipo::build_layer_ffn(ggml_tensor * cur, const int il) {
    // Check if this is an MoE layer
    if (ffn_gate_inp != nullptr) {
        // MoE branch
        ggml_tensor * moe_out =
            build_moe_ffn(cur,
                ffn_gate_inp, ffn_up_exps,
                ffn_gate_exps, ffn_down_exps,
                nullptr,
                n_expert, n_expert_used, LLM_FFN_SILU,
                true, false, 0.0, LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il,
                nullptr, ffn_gate_up_exps);
        ...
        if (ffn_up_shexp != nullptr) {
            ggml_tensor * ffn_shexp =
                build_ffn(cur,
                    ffn_up_shexp,   NULL, NULL,
                    ffn_gate_shexp, NULL, NULL,
                    ffn_down_shexp, NULL, NULL,
                    NULL,
            ...
            ggml_tensor * shared_gate = build_lora_mm(ffn_gate_inp_shexp, cur);
